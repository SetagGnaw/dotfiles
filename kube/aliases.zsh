...
alias kgpol='kubectl get pods -l' #  kgpol 'app=nginx,env=prod'
...
# label
# kl  — set/update one or more labels (space-separated key=value pairs)
#   kl pod my-pod env=prod
#   kl pod my-pod env=prod tier=backend        # multiple at once
#   kl pods --all env=prod                     # all pods in namespace
#   kl pods -l app=nginx env=prod              # all pods matching selector
#   kl pods -l 'env in (staging,dev)' env=prod # set expression selector
alias kl='kubectl label'

# klao — same as kl but overwrites existing keys without error
#   klao pod my-pod env=staging
#   klao pod my-pod env=staging tier=frontend
#   klao pods -l app=nginx env=prod             # overwrite on matching pods
alias klao='kubectl label --overwrite'

# klarm — remove one or more labels from a resource (append '-' to each key)
# Usage:  klarm <resource-type> <name> <key> [key2 ...]
# Example:
#   klarm pod my-pod env
#   klarm pod my-pod env tier               # remove multiple labels at once
#   klarm node node-1 dedicated
